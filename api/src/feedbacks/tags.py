...
class FeedbackTagsResult(SQLModel):
    apiVersion: str = "v1alpha1"
    kind: str = "FeedbackTagsResult"
    items: List[Item]
...
def get_tags_generic(session: Session = Depends(get_session)) -> FeedbackTagsResult:
    ...
    return FeedbackTagsResult(items=items)
...
def get_tags_generic2(session: Session = Depends(get_session)) -> FeedbackTagsResult:
    statement = (
        ...
        .where(FeedbackEntity.tags != null())
    )
...
def get_tags_generic3(session: Session = Depends(get_session)) -> FeedbackTagsResult:
    statement = (
        ...
        .select_from(FeedbackEntity)
        .group_by(col(FeedbackEntity.tags))
        .where(FeedbackEntity.tags != null())
    )
...
def get_tags_sqlite(session: Session = Depends(get_session)) -> FeedbackTagsResult:
    statement = text(
